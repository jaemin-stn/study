...
function App() {
  const { addRack, loadState, selectedRackId, racks, isEditMode, setEditMode } = useStore();
  ...
  return (
    <div style={{ width: '100%', height: '100vh', position: 'relative' }}>
      ...
      <div style={{
        ...
        <div style={{ marginLeft: '10px', fontSize: '12px', color: '#666', alignSelf: 'center' }}>
          Use Left Mouse to Rotate, Right to Pan. Click Rack to Edit. Click Error Marker to Fly-to.
        </div>
      </div>

      {/* Side Panel */}
      {selectedRackId && (
        <div style={{ position: 'absolute', top: 0, right: 0, height: '100%', zIndex: 20 }}>
          <DevicePanel />
        </div>
      )}
    </div>
