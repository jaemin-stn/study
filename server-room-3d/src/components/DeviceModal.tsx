...
export const DeviceModal = () => {
    const { racks, selectedDeviceId, selectDevice } = useStore();
    ...
    const renderPort = (portIndex: number) => {
        ...
        const shadowColor = error ? 'rgba(239, 68, 68, 0.4)' : 'rgba(34, 197, 94, 0.2)';
        ...
        return (
            <div
                ...
                style={{
                    ...
                    position: 'relative',
                    cursor: 'pointer',
                    boxShadow: error ? `0 0 10px ${shadowColor}` : 'none'
                }}
                title={error ? `${portId}: ${error.errorMessage}` : `${portId}: Operational`}
            >
                <div style={{
                    width: '12px',
                    height: '10px',
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h2 style={{ color: '#fff', margin: 0, fontSize: '20px' }}>{device.name}</h2>
                        <span style={{ color: '#888', fontSize: '12px' }}>Type: {device.type} | Rack: {rackId?.substring(0, 4)}</span>
                    </div>
                    <button
                        onClick={() => selectDevice(null)}
                ...
                {device.portStates.length > 0 && (
                    <div style={{
                        ...
                        <h4 style={{ color: '#ef4444', margin: '0 0 8px 0', fontSize: '14px' }}>Active Faults</h4>
                        {device.portStates.map((err, idx) => (
                            <div key={idx} style={{ color: '#fff', fontSize: '13px', marginBottom: '4px' }}>
                                <strong>{err.portId}</strong>: {err.errorMessage} ({err.errorLevel})
                            </div>
                        ))}
                    </div>
                )}
