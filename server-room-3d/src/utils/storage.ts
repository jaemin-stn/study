...
export const sampleRacks: Rack[] = Array.from({ length: 20 }).map((_, i) => {
    ...
    for (let d = 0; d < 5; d++) {
        ...
        const fittingTemplates = DEVICE_TEMPLATES.filter(t => t.uSize <= remainingU);

        if (fittingTemplates.length === 0) break;

        const template = fittingTemplates[Math.floor(Math.random() * fittingTemplates.length)];
        devices.push({
            ...
            portStates: Math.random() > 0.7 ? [
                {
                    portId: `p${Math.floor(Math.random() * 24) + 1}`,
                    status: 'error' as const,
                    errorLevel: (['warning', 'minor', 'major', 'critical'] as const)[Math.floor(Math.random() * 4)],
